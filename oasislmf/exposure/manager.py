...
from ..utils.il import (
    combined_grouped_canonical_profile,
    get_level_il_terms,
    get_policytc_ids,
)
...
class OasisExposureManagerInterface(Interface):  # pragma: no cover
    ...
    def generate_fm_items(self, canonical_exposures_df, gul_items_df, canonical_exposures_profile, canonical_accounts_profile, canonical_accounts_df, il_agg_profile, **kwargs):
        """
        Generates FM items.
        ...
        :param il_agg_profile: FM aggregation profile
    ...
    def load_fm_items(self, canonical_exposures_df, gul_items_df, canonical_exposures_profile, canonical_accounts_profile, canonical_accounts_file_path, il_agg_profile, **kwargs):
        """
...
class OasisExposureManager(implements(OasisExposureManagerInterface)):
    ...
    def load_fm_aggregation_profile(self, oasis_model=None, **kwargs):
        ...
        profile_path = kwargs.get('il_agg_profile_path')
        profile_json = kwargs.get('il_agg_profile_json')
        ...
        if oasis_model:
            oasis_model.resources['il_agg_profile'] = profile
    ...
    def _process_default_kwargs(self, oasis_model=None, **kwargs):
        if oasis_model:
            ...
            kwargs.setdefault('il_agg_profile', omr.get('il_agg_profile'))
            kwargs.setdefault('il_agg_profile_path', omr.get('il_agg_profile_path'))
            kwargs.setdefault('il_agg_profile_json', omr.get('il_agg_profile_json'))
    ...
    def generate_fm_items(
        ...
        try:
            ...
            for it in (it for c in itertools.chain(six.itervalues(preset_items[k]) for k in preset_items) for it in c):
                ...
                lfmaggkey = ilap[it['level_id']]['FMAggKey']
    ...
    def load_fm_items(
        ...
        try:
            ...
            fm_items = [it for it in self.generate_fm_items(canexp_df, gul_items_df, cep, cap, canacc_df, ilap)]
    ...
    def write_fm_files(self, oasis_model=None, **kwargs):
        ...
        if oasis_model:
            ...
            il_agg_profile = omr['il_agg_profile']
        else:
            ...
            il_agg_profile = kwargs.get('il_agg_profile')

        fm_items_df, canacc_df = self.load_fm_items(canexp_df, gul_items_df, canonical_exposures_profile, canonical_accounts_profile, canonical_accounts_file_path, il_agg_profile)
    ...
    def start_oasis_files_pipeline(
        ...
        **kwargs
        ...
        if fm:
            ...
            il_agg_profile = kwargs.get('il_agg_profile') or self.load_fm_aggregation_profile(oasis_model=oasis_model, **kwargs)

            if il_agg_profile is None:
                raise OasisException('FM option indicated but no FM aggregation profile provided')

            logger.info('\nLoaded FM aggregation profile: {}'.format(il_agg_profile))
    ...
    def create_model(self, model_supplier_id, model_id, model_version, resources=None):
        ...
        if omr.get('il_agg_profile') is None:
